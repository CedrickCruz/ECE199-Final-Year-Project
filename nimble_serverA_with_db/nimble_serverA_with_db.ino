...
void getLocation(int rssi_a, int rssi_b, int rssi_c) {
    ...
    for (int i = 0; i < NUM_LOCATIONS; i++) {
        ...
        int distanceB = 0;
        int distanceC = 0;
        distanceA += abs(rssi_a - RSSI_DB[i][0]);
        distanceB += abs(rssi_b - RSSI_DB[i][1]);
        distanceC += abs(rssi_c - RSSI_DB[i][2]);

        LOCATION_DIFF_A[i] += distanceA;
    ...
    estimatedLocation = estimate_location(RSSI_AVERAGE, LOCATION_NAMES);
