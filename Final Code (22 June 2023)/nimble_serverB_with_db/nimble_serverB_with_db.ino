...
void setup() {
    ...
    // Set BLE TX Power to +9dBm
    esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_DEFAULT, ESP_PWR_LVL_P9);
    esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_ADV, ESP_PWR_LVL_P9);
    esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_SCAN, ESP_PWR_LVL_P9);

    int pwrAdv = esp_ble_tx_power_get(ESP_BLE_PWR_TYPE_ADV); // Should show index 7 (+9dBm) txPower
...
void loop() {
    if (deviceConnected) {
        ...
        // when RSSI is 0, this means that the beacon is disconnected
        if (rx_a < 0) {
