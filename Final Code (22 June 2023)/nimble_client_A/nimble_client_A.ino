...
boolean tag_codeA = true;
boolean tag_codeB = true;
boolean tag_codeC = true;
...
void deep_sleep(){
    ...
    esp_sleep_enable_timer_wakeup(SLEEP_TIME_SECONDS * 1000000); //NOTE: 1000000 is uS to S conversion factor
...
void setup() {
    ...
    pBLEScan->setInterval(40);  //millisec (how often) tested: 1349, 45, 20, 1280, 10240
    pBLEScan->setWindow(20);     //millisec (how long) tested: 449, 15, 20, 1280, 10240
...
void loop() {
    ...
    if (connectedA) {
        // Set tag code for status in app
        if(tag_codeA) pRemoteCharacteristicA->writeValue("A");
        tag_codeA = false;
        ...
        String string_received = String(rxValue.c_str());         // Format to be received "tag, command" e.g. "A,1" from tag A with command "1"
        int commaIndex = string_received.indexOf(',');
        string_received = string_received.substring(commaIndex + 1);  // Sets variable to command for reading

        // Read data is "1" corresponds to Scan
        int rx_int = atoi(string_received.c_str());
        if (rx_int == 1) {
            sleep_count = 0;
        ...
        // Read data "2" corresponds to Ring
            ...
            ledcWriteNote(PWM_CHANNEL, NOTE_B, 4);                                 // Configure note to play
        ...
        // Enter deep sleep to reset connection
        else if (rx_int == 3){
            tag_codeA = true;
            //pRemoteCharacteristicA->writeValue("A,Waiting for command...");
            deep_sleep();
        }
    }
    if (connectedB) {
        // Set tag code for status in app
        if(tag_codeB) pRemoteCharacteristicB->writeValue("A");
        tag_codeB = false;
        // Read Characteristic value of Server
        ...
        // Enter deep sleep to reset connection
            ...
            tag_codeB = true;
            //pRemoteCharacteristicB->writeValue("A,Waiting for command...");
            deep_sleep();
        }
    }
    if (connectedC) {
        // Set tag code for status in app
        if(tag_codeC) pRemoteCharacteristicC->writeValue("A");
        tag_codeC = false;
        // Read Characteristic value of Server
        ...
        // Enter deep sleep to reset connection
            ...
            tag_codeC = true;
            //pRemoteCharacteristicC->writeValue("A,Waiting for command...");
            deep_sleep();
        }
    }
